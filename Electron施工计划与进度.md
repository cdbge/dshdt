...
| 17 | **更新复测（门禁挡住）+ junction 场加固 + 0.4.5 安装核查** | 🟡 代码完成待热更新（0.1.5 更新仍被门禁挡，真因已定位） | **① 复测更新**：用壳自己的 API 走 `检查 → 更新（allowUnsafeJump）`，构建全程通过（ABI `OK=5 FAIL=0`），但**启动门禁未通过**。**真因实测定位**：0.1.5 的根 URL 是 **303 换 cookie**——裸 URL→401、带 token 但默认跟随重定向→401（**Node 的 fetch 没有 cookie jar，303 跳转丢掉 `Set-Cookie`**）、带 token + `redirect:'manual'`→303+`Set-Cookie: dsh-auth-…`、**带上该 cookie 请求 `/` → 200（28029 字节完整 SPA）**。所以 `runBootGate` 与壳的 `waitReady` 都只认 `res.ok`，**对任何 0.1.5+ 的树都会永远探不到就绪**（症状与原始事故一致）——第一轮只加了"token 感知的 URL 提取"，没解决换票这一跳。**② junction 加固**：新增 `src/junction-safe.mjs`（`safeRemoveTree` 逐个 unlink 链接、绝不递归进目标；`cleanStaleBootGateHomes` 收遗留隔离目录，判年龄用 **birthtime**），接入门禁 finally 与壳启动。**③ 0.4.5 安装核查**：顶层每个文件与 `dist\win-unpacked` **逐字节同尺寸**（`DSH Desktop.exe` 225,663,488），**安装其实完整**；安装程序拒绝运行是对的——`DSH Desktop.exe` 有 6 个进程在跑（含监听 25439 的壳与 3547 的宿主），用户按 "dsh" 搜不到是因为**壳的宿主子进程就叫 `DSH Desktop.exe`** | **三个假设全部被实验证伪**（好过直接断言）：`fs.rmSync` 不跟随 junction（最小复现）；门禁不破坏被测树（拿现网拷贝跑真门禁，`15210→15210`）；0.1.5 宿主不破坏自己的树（`11173→11173`）。**未证实但极强的线索**：损坏形态"**目录还在、文件全没**"是 `del /s /q` 的特征，而 `$DSH_HOME\profiles\node_modules` 有**199 个 junction 指向现网 vendor**（含整个 `@deepseek-ai`）——一次跟随 junction 的删除就能一次性掏空 240 个包。坏树被我自己的探针脚本开头 `rmSync(staging)` 删掉了，**无法再取证**（失误已记）。**门禁：167 离线断言 0 失败**（update 36 / vendor-build 54 / dsh-apply 44 / junction-safe 18 / repair 8 / admin-bg 7）。**遗留**：token 探针未修（改完需热更新）；0.1.5 仍不可更新 |
