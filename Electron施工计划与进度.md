...
| 4 | M2 构建与分发（剪枝 + electron-builder + 签名 + 更新器） | ✅ 完成 | **签名安装包产出**：DSHDesktop-Setup-0.4.0.exe（146.8MB，自签 CN=DSH Desktop Dev）+ blockmap；打包产物 smoke exit 0；dev 21/21；CI 工作流 + updater 接线就位 | 发布源待激活：建 GitHub 仓库 → yml 填 publish → Secrets 配 GH_TOKEN → tag 触发（README 有步骤） |
