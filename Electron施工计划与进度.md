...
| 14 | **DSH 更新按钮 S1+S2：更新引擎 + vendor 构建原语** | 🔄 进行中（S1/S2 完成，S3~S5 未开始） | **S1** `src/dsh-update.mjs`（版本发现与比较）+ `scripts/update-self-test.mjs`（29 断言）；**S2** `src/vendor-build.mjs`（npm 探测/install/剪枝/插件同步/ABI 门禁/lock/`buildStaging`）、`build-host.mjs` 与 `abi-scan.mjs` 退化为 CLI 薄封装、`vendor-build-self-test.mjs`（49 断言）、`vendor-equivalence.mjs`（暂存树验证门禁）。门禁合计 **93 断言 0 失败**；**真实端到端**：完整 npm install（441s）→ 剪枝 16580 文件 → ABI `OK=5 FAIL=0` → **暂存树真起宿主，就绪 URL + `GET /` 200 + 优雅关停**，全程未触碰现网 vendor | **Q1** 换树须征得用户同意（开发期逐次）；**Q2** 不内置 npm；**Q3** registry=npmmirror；**Q4** 不做回滚备份（有安装包可重装）→ 门禁成唯一防线。**关键发现**：`electron-builder` 的 `files` 只含 `src/**`，**`scripts/` 不进包**，故门禁代码必须落 `src/`；`vendor.lock.json` 的体积基线是 **pnpm** 建的（含 `.pnpm`/`pnpm-lock.yaml`），拿 npm 树比它文件数差 14.4% 属预期，**判据须改为功能性**（ABI + 启动冒烟）。踩坑：单测夹具/断言命名两次把脚本 bug 伪装成产品缺陷，已改正并回填规范坑 27/28 |
