...
| 29 | **根因落定：注册顺序 —— 必须 `{ prepend: true }` 抢在 DSH 的审批桥之前** | ✅ 已修 + 已部署（**待重启验证端到端**）；机制已用动态插件**当场证实** | 用动态 Cordis 插件做对照实验（宿主半身 `ctx.on` + `console.log` → 落 `host.log`，**不需要重启**）。**第一轮探针**（同时挂 `plain` 与 `{global:true}` 两个监听）：**收到 0 条**（卸载时打印 `累计 plain=0 global=0`）⇒ 连 `global` 也没用，说明前面两轮的思路都不对。**读 `waterfall()` 找到真机制**：`const next = () => (cbs.shift() ?? inner)(...args)` —— 瀑布**按注册顺序**执行；而 DSH 自己的桥 `dsh-api-remotes` 是 `return ctx.on(event, function(request, next) { … return forwardWaterfall(…) })` —— 它把请求转发给浏览器、拿回用户点击结果就**直接返回、不调 `next()`**，**整条链到此终止**。⇒ **注册在桥之后的监听永远不会被执行**：插件静默失效、用户照旧看到卡片、照旧手点（第 28 行实测：54 次全手点）。**第二轮探针**（改成 `{ prepend: true }`，**只认领 justification 里带特定标记的请求**，其余一律 `next()`）：提权实测 **`claimed=1 passed=0`** —— 抢到了、当场放行、**用户零点击** ⇒ 机制证实。**这也解释了历史**：早先插件排在桥前面（挂载顺序不同），所以有过 36 条记录；某次挂载顺序变了它就哑了，而"装载成功"一切照旧，从外面看不出异样。**修法**：`ctx.on('approval/request', handler, { global: true, prepend: true })` —— 两个维度缺一不可：`prepend` 管**顺序**、`global` 管**作用域** | **门禁：离线 210 → 211 断言**（新增断言"监听声明了 `{ prepend: true }`"）、smoke **71/71**；三份副本哈希一致（`6755FA18…`）。动态探针已 `cordis_stop`（定义保留） | **教训入规范坑 50（标题与结论已重写为真正的根因）**：① **`ctx.on` 成功 ≠ 收得到** —— 它不抛错、fiber 也 active，事件也可能永远轮不到你；② **读源码猜不出顺序，就上动态插件当场测** —— 本轮两次"读源码得出确定性结论"都被实测证伪，而动态插件几分钟就给出了答案，**这类接线级疑问应优先用可观测实验回答**；③ 保留第 28 行的假阴性教训（`zstdDecompressSync` 对多帧文件只解第一帧就"成功"返回）。**遗留**：插件源码不热加载，**需重启一次**才能让 v2（独立模型审查）真正跑起来 —— 重启后一次提权即可验收：日志应出现 `source=model` 的决策条目，且**不再弹卡片** |
