...
| 13 | **0.4.7：自带插件 + 托盘重启宿主** | ✅ 完成 | `dsh-auto-approval` 进仓库 `packages/`（build-host 拷进 vendor、壳启动自动挂载 profile）；托盘「重启宿主（重载插件）」+ `POST /api/restart-host`；门禁：admin-bg 7/7、repair 8/8、插件自检 10+12、smoke **40/40** | 未打包；已装应用需**重启一次**才有托盘按钮（asar 只在启动时加载） |
