...
| 4 | M2 构建与分发（剪枝 + electron-builder + 签名 + 更新器） | 🔄 进行中 | **安装包已产出并通过打包产物 smoke**（DSHDesktop-Setup-0.4.0.exe 146.8MB + blockmap；win-unpacked --smoke exit 0；isPackaged 路径适配实测 OK）；dev 21/21 恢复全绿 | ⚠️ 关键坑已解：`import { autoUpdater } from 'electron-updater'` CJS 命名导入在 Node 24 抛错（打包态弹对话框挂起）→ 改默认导入；剩：签名（自签）+ GitHub Releases 发布源 |
