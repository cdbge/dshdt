...
| 31 | **让审查器"读命令"（补上第 30 行的遗留）+ 内置同步到 vendor 树，准备打包** | ✅ 完成（用户实测确认"确实没弹"） | **① 读命令**：审批请求里只有 `{toolName, reason, callId}`、**没有 args** —— 所以审查器此前只能"读理由"，而实测证明那正是最容易被误导处（模型自己抱怨过"理由与动作不匹配"）。接口依据来自 Inspect 的 `ToolExecutionInput` 声明：**`arguments: unknown`** 就是真实参数，而 `tools/pre-execute` 在沙箱提权**之前**触发。做法：在那里记一张 `callId → {name, args}` 的**有界小表**（上限 20 条），审批时按 `callId` 取回；新增纯函数 `summarizeArgs(args, limit)`（序列化 + 截断 + 标注原文长度）把它压成可读文本。喂给模型 1200 字、审计日志留 300 字预览（事后能直接对比"规则看到的理由"与"实际要跑的命令"）。系统提示同步加了一条：**`command` 是真实参数、`reason` 只是智能体自述，冲突时以 `command` 为准并倾向 ask**。`tools/pre-execute` 监听同样要 `{global:true}`（作用域），但**不 prepend** —— 它只观察、不裁决 | **门禁：离线 211 → 219 断言**（`apply-self-test` **29 → 37**：新增 `summarizeArgs` 三条、打包含命令字段两条、**端到端"审查器拿到真实命令"**、无记录时为空串的 fail-safe、以及 pre-execute 监听选项断言）；smoke **71/71**；插件已部署两份副本（哈希一致） | **② 内置与打包准备**：确认壳早已把插件当内置（`PROFILE_PLUGIN_NAMES` / `ensureProfilePluginMount` / `DEFAULT_PLUGIN_NAMES` 三处）；`electron-builder.yml` 用 `extraResources: from: vendor → to: vendor` 把整棵 vendor 树放进包（`files` 只含 `src/**`+`VERSION`+`package.json`）。**关键发现：仓库 vendor 树里的插件是旧的**（不含 `prepend`、不含读命令）—— **直接打包会把带 bug 的版本发出去**。已用 `build-host.mjs --prune-only` 同步（该次调用被中断，但**关键步骤已完成**：196 包完好、插件哈希与仓库一致、`vendor.lock.json` 已刷新且 `abiScan: PASS`）。VERSION 与 package.json 均为 **0.4.6** | **打包命令**（`desktop-electron/` 下）：`npm run dist`（= `electron-builder --win nsis`）。**遗留**：① 尚未真正执行打包（用户说的是"准备打包"）；② 打包产出的 exe 内含的是**当前 vendor（rc.8）**，而**已装应用跑的是 0.1.5-rc.2** —— 两者不一致这件事仍未处理（坑 45）；③ 已装应用要拿到"读命令"这版插件仍需**重启宿主**一次 |
