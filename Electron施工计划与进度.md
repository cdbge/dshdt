...
| 14 | **DSH 更新按钮 S1：更新引擎（版本发现与比较）** | 🔄 进行中（S1 完成） | 新增 `src/dsh-update.mjs`（纯 Node、零 Electron 依赖）+ `scripts/update-self-test.mjs`（**36 断言全绿**，离线不联网）；门禁：`node --check` OK、repair 8/8、admin-bg 7/7 无回归。计划书《DSH更新按钮计划书.md》§12 记录 Q1~Q4 决策 | **Q1** 换树须征得用户同意（开发期逐次）；**Q2** 不内置 npm（探测系统 Node，缺失则按钮置灰）；**Q3** 默认 registry = npmmirror；**Q4** 不做回滚备份（用户有安装包可重装）→ marker 前置门禁成唯一防线。**S2~S5 未开始**（staging 构建 / admin+主进程 / UI / 门禁与文档）。踩坑：单测夹具用了 `9.9.9-rc.1` 导致"是否有更新"断言恒假，把脚本 bug 伪装成产品缺陷——正是规范 §24 的教训，已改正 |
