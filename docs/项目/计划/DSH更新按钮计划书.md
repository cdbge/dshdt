...
> 配套文档：`docs/通用/`（4 份通用规范）、`docs/项目/02-架构与铁律.md` 与 `docs/项目/03-坑清单.md`（本项目硬约束）、`docs/项目/计划/Electron施工计划与进度.md`（唯一进度事实源）。
...
引自 `docs/项目/01-新会话开工清单.md` §0，若与仓库实际不一致以仓库为准：
...
- `docs/项目/计划/Electron施工计划与进度.md`：进度快照表加行 + 阶段小节（唯一进度事实源）
- `docs/项目/02-架构与铁律.md`：§2 工程地图加 `src/dsh-update.mjs` 行；**铁律 3 扩展「DSH 换树」（Q1 决定）**；`docs/项目/03-坑清单.md` 回填本期新坑（预计至少 3 条：registry prerelease 比较、rename 换树时序、`DSH_BIN` 顶层求值）
- 通用文书（`docs/通用/`）：新坑若同样适用于别的项目，按 `docs/项目/00-文档导航.md` §4 的规则同步一份
...
| # | 问题 | **决定** | 对方案的影响 |
|---|---|---|---|
| **Q1** | DSH 换树的同意口径 | **换树必须征得用户同意** | 采用最保守口径：**开发/测试期每一次真实换树都要先获得用户明确同意**，绝不自行触发。正式使用中用户点击按钮即为同意，不额外弹二次确认。→ 需把该口径**写入 `docs/项目/02-架构与铁律.md` 铁律 3**（见 §9） |
