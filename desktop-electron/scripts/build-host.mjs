// vendor/profile 自包含构建：锁定 rc.6 依赖安装 → ABI 门禁 → 轻量剪枝 → vendor.lock.json
// 用法：node scripts/build-host.mjs [--runtime <electron.exe 路径>]
...
if (!npm) { console.error('[build-host] 找不到 npm，终止'); process.exit(1) }
console.log(`[build-host] npm 入口: ${npm.cmd} ${npm.args.join(' ')}`)
console.log('[build-host] npm install --omit=dev --ignore-scripts（首次约 255MB 依赖，耐心等待）...')
const install = spawnSync(npm.cmd, [...npm.args, 'install', '--omit=dev', '--ignore-scripts', '--no-audit', '--no-fund'], { cwd: PROFILE_DIR, stdio: 'inherit', env })
if (install.status !== 0) {
  console.error(`[build-host] npm install 失败（status=${install.status} error=${install.error?.message ?? 'none'}），终止`)
  process.exit(1)
}

// 3. ABI 门禁：win32-x64 平台包在 Electron 运行时下全部 dlopen 成功
...
// 4. 轻量剪枝：node-pty 包内自带的非 win32-x64 预编译
let prunedBytes = 0
for (const p of ['darwin-arm64', 'darwin-x64', 'win32-arm64']) {
  const d = path.join(PROFILE_DIR, 'node_modules', 'node-pty', 'prebuilds', p)
  if (fs.existsSync(d)) { prunedBytes += dirSize(d); fs.rmSync(d, { recursive: true, force: true }) }
}
console.log(`[build-host] 剪枝 node-pty 非本平台预编译: ${mb(prunedBytes)}`)
