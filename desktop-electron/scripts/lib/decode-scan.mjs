// decode-scan.mjs — JPEG 单扫描段解码（jpeg-decode.mjs 的熵解码部分）。
// 同一份循环骨架按扫描头参数分派三种形态：基线一次解完 0..63；渐进首扫（ah===0）写初值；
const ZIGZAG = [
...
// 因此不必猜数据边界。段内仍需处理字节填充：FF 00 表示字面量 0xFF。
...
export function decodeScan({ buf, sc, frame, huff, restartInterval, progressive, byId, mcusX, mcusY, coeffsOf, predOf, onDiagnostic = () => {} }) {
  ...
  // 每个扫描重置一次：否则前一个扫描会把配额占满，失败的那个扫描一次都看不到
  trace.count = 0
        ...
        for (const cs of sc.comps) {
          ...
          for (let by = 0; by < comp.v; by++) {
            for (let bx = 0; bx < comp.h; bx++) {
              ...
              // 渐进式细化扫（ah > 0）：按规范算法写（P1/M1 双掩码 + 位置式细化 + EOBRUN 跨块递减）。
              // 别"简化"成先读符号再统一修正——那样每块的掩码会被反复重置、同一系数加两次，画面块状且整体偏亮。
              const p1 = 1 << sc.al
              const m1 = (-1) << sc.al
