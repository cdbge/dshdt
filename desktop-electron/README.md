...
| `scripts/ci-self-test.mjs` | 168 | **CI 配置与门禁清单自查**（三平台矩阵、产物 glob、清单与磁盘一致、打包配置、**递归删除入口**、交叉构建延后契约、**macOS 双架构前置条件**、**打包前 vendor 平台核对**、**shell 脚本行尾/BOM**、**`.icns` 纯 Node 生成不许退回 macOS-only**、**跨平台内容一致性（版本锁）**、**托盘三条"打开主窗"入口**、README 数字） |
