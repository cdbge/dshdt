...
**Windows 10/11（x64）与 Linux（x64，AppImage/deb）已实测并出包；macOS（arm64/x64，dmg/zip）有构建链路但未在真机验证，且不阻塞发布**。
...
| `scripts/ci-self-test.mjs` | 169 | **CI 配置与门禁清单自查**（三平台矩阵、产物 glob、清单与磁盘一致、打包配置、**递归删除入口**、交叉构建延后契约、**macOS 双架构前置条件**、**打包前 vendor 平台核对**、**shell 脚本行尾/BOM**、**`.icns` 纯 Node 生成不许退回 macOS-only**、**跨平台内容一致性（版本锁）**、**托盘三条"打开主窗"入口**、README 数字） |
...
| `scripts/shell-hot-update-self-test.mjs` | 32 | **按钮热更新 dshdt 自身**（1.0.0 的唯一功能）：asar 补丁器（只换指定文件、**保留 asar 里的 node_modules**、**重算 integrity**、`unpacked` 条目原样留、路径穿越拒绝、超单块上限拒绝）＋**独立裁判交叉验证**（拿 `@electron/asar` 读回我们写的 asar；本地有 `dist/` 时在**真实产物**上打补丁并逐条目比字节——285 条一个不差）＋换壳编排（暂存源码 → 打新 asar，**现网文件不动**）＋可写性探测（Linux deb/AppImage 必须如实拒绝）＋**助手脚本端到端**：真跑一遍"换件 → `--smoke` 校验 → 重启"，**成功与失败回滚两条路都测**（回滚路径断言坏包被留下、备份归位、旧壳被启动回来）＋壳/界面/托盘接线 |

| `scripts/ci-shell-syntax-self-test.mjs` | 3 | **CI shell 脚本自己的语法门禁**（v1.0.0 首次发布翻车换来的）：`ci-run.sh` 里多出一个双引号 ⇒ bash 以 **exit 2** 拒绝执行整个脚本 ⇒ 三平台所有包着它的步骤（生成图标 / 开发态 smoke / 打包 / asar 门禁）在同一秒全部失败，而**注解一条都没有**——因为打印注解的脚本自己没跑起来。两层判据：① 引号/反引号**配平扫描**（小状态机；单引号内的内容不计，否则 `echo '```'` 会假红）；② `bash -n` 真语法检查（本机没有 bash 就**如实标注跳过**，绝不把"没检查"报成"检查通过"） |

口径：跨平台改造前是 242 断言（9 套）；当前 **30 套**（以 `npm run test:suite` 的输出为准）。
