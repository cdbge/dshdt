...
**支持平台**：Windows 10/11（x64）与 Linux（x64，AppImage/deb）已实测并出包；macOS（arm64/x64，dmg/zip）
有构建链路但未在真机验证，且不阻塞发布。三平台差异集中在 `src/platform-paths.mjs` 与 `vendor` 树（必须按
目标平台构建）；构建链路在 `scripts/`（`build-host.mjs` 建树、`linux/` 出 Linux 包、`build-mac-universal.mjs`
合并双架构）。
...
│  ├─ platform-paths.mjs      #   平台路径与二进制解析（三平台 data/log/home/workspace + electron 可执行文件）
...
## 开发运行
前提：Node.js 22+、构建 vendor 时可访问 npm registry。**支持 Windows / Linux / macOS**：壳侧平台差异集中在
...
| `scripts/ci-self-test.mjs` | 169 | CI 配置与门禁清单自查：三平台矩阵、产物 glob、清单与磁盘一致、打包配置、递归删除入口、交叉构建延后契约、macOS 双架构前置条件、打包前 vendor 平台核对、shell 脚本行尾/BOM、`.icns` 纯 Node 生成、跨平台内容一致性（版本锁）、托盘三条"打开主窗"入口、README 数字 |
| `scripts/dsh-apply-self-test.mjs` | 44 | 换树与标记状态机（全同步，每条失败分支一条断言） |
| `scripts/cross-tree-self-test.mjs` | 43 | vendor 树静态体检判据 + 双架构合并（缺件 / 串平台 / 假绿 / 并入架构缺件 / 跨平台+跨架构污染都必须报错） |
...
| `scripts/tray-icon-self-test.mjs` | 20 | 托盘/窗口图标平台判据 + 三条"打开主窗"入口：Linux/macOS 的图标解码**不认 `.ico`**、Linux 上 `double-click` 事件不存在、菜单第一项必须是"打开主窗口" |
...
| `scripts/shell-hot-update-self-test.mjs` | 32 | 按钮热更新 dshdt 自身：asar 补丁器（只换指定文件、保留 asar 内 `node_modules`、重算 integrity、拒绝路径穿越与超单块上限）+ `@electron/asar` 充当独立裁判读回校验 + 换壳编排（换新 asar 时现网文件不动）+ 可写性探测（Linux deb/AppImage 必须如实拒绝）+ 助手脚本端到端（成功与回滚两条路都测） |
...
### CI（三平台矩阵）
`../.github/workflows/release.yml` 有四个 job：`self-test`（三平台 matrix 各跑一遍 `test:suite` + 版本锁 strict
核验）、`windows`（NSIS）、`linux`（AppImage+deb）、`macos`（dmg+zip，arm64+x64）；推 `v*` tag 或手动
...
```
`--strict` 是完整判据（能拦住 DSH 版本变了、清单多了依赖、锁缺失、锁损坏四种漂移），CI 里挂在三个打包 job
**之前**的 `self-test` job 上——锁不对，三平台产物就不该被生产出来。

签名 Secrets（未配置则出未签名包）：Windows `WINDOWS_CERT_PFX` / `WINDOWS_CERT_PASSWORD`；macOS `CSC_LINK` /
`CSC_KEY_PASSWORD` 与公证用 `APPLE_ID` / `APPLE_APP_SPECIFIC_PASSWORD` / `APPLE_TEAM_ID`。
...
（CLI 薄封装，实现在 `src/`，支持 `--os/--cpu/--libc` 交叉构建）、`scripts/gen-icon.mjs`（三平台图标 ico/png/icns）、
`scripts/dev-sign.ps1`（Windows 自签）。

## 构建与打包
三平台各自在目标平台上执行（`build:host` 产出的是**该平台**的 vendor 树）：
...
npm run build:mac-universal && npm run dist:mac   # → dist/DSHDesktop-<版本>-<arch>.dmg + .zip
# 必须用 build:mac-universal（不是 build:host）：单棵树会让 x64 产物装上也起不来
```
...
- **macOS 必须用 `build:mac-universal`**：`--mac dmg zip --arm64 --x64` 会把同一棵 vendor 树打进两个架构的 .app，
  而 `npm install` 一次只能按一个 `--cpu` 解析可选依赖 ⇒ 单棵树会让 x64 的 .app 装着 arm64 的 `koffi`/`node-pty`
  （import 期崩）。`node scripts/build-mac-universal.mjs --host arm64 --add x64` 先建宿主架构那棵，再建另一架构那棵当
  donor，只并入其**平台专属**条目（脚本与 JS 同源，整树拷贝会引入两份可能漂移的代码）；合并后 lock 多一个
  `mergedPlatforms` 段，体检脚本对该架构也逐项验包。体积代价：macOS 树 125.9 MB（单架构 99.9 MB，多出的 26 MB 是两套 `libvips`）。
...
- **在哪台机器上能打出哪个包**：
  ...
  | Linux `AppImage` | ❌ | ✅ | 要 `mksquashfs`（electron-builder 的 appimage 包只有 `darwin/`+`linux/` 两个目录） |
  | Linux `deb` | ❌ | ✅ | 要 `fakeroot`/`rpm`/`ar`（`ar` 来自 `binutils`，漏了报 `Need executable 'ar' to convert dir to deb`） |
  | macOS `dmg`/`zip`/`dir` | ❌ | ❌ | **硬边界**：electron-builder 拒绝非 macOS 主机（`Build for macOS is supported only on macOS`），只能靠 macos runner 或真 Mac |

- **macOS 产物在 Mac / macos runner 上按序跑**：

  ```bash
  npx electron scripts/gen-icon.mjs                  # 含 .icns
  ...
  ```
  再对 `dist/mac*/*.app` 跑打包态冒烟（判据同 Windows：按输出里的 `PASS` 计数）。
- **在 WSL 里出 Linux 安装包**：
...
- **`.icns` 由纯 Node 生成**（不需 macOS 的 `sips`/`iconutil`）：`.icns` 是"容器 + 若干 PNG"；核验在
  `scripts/verify-icns.mjs`（头/总长/成员表严丝合缝、每个成员是合法 PNG 且宽高与类型码一致、7 个档位齐全）。
- **三平台包内容一致靠一份版本锁**：manifest 只钉三个 DSH 包的精确版本，**传递依赖是范围声明**
  （`zod: ^4.4.3`、`node-addon-require-builtin: ^0.1.4`、`@types/node` 由 `protobufjs` 的 `>=13.7.0` 拉进来）。仓库里放
  一份**平台中立**的 `vendor/package-lock.json`（587 个包，含全部平台专属条目），三平台都用它装；装前把锁拷进
...
- **包体为什么大**：安装包内自带整棵 DSH 运行时（Windows 104.4 MB / Linux 103.1 MB / macOS 单架构 99.9 MB·双架构
  125.9 MB）。安装耗时主要跟**文件数**相关（解压 + Defender 逐文件扫描），所以剪枝优先砍文件数而不是字节数
  （`.map` / `.d.ts` / `.md` / 许可证 / `.pdb` 都会剪掉）。
- **签名**：Windows 未签名时首次运行有 SmartScreen 提示（本地开发可自签）；macOS 未签名/未公证时需右键"打开"，
  自更新要求已签名。entitlements 模板在 `build/entitlements.mac.plist`。
...
**为什么 macOS/Linux 要拷一份**：DSH 更新按钮要"换树"，而换树靠目录改名（rename）。macOS 的包内内容不可改
（改了就**破坏代码签名**，开 hardened runtime/公证后甚至拒绝启动），Linux AppImage 的 `resources` 是**只读挂载**、
deb 装到 `/opt` 又属 root —— 所以可变的那棵树必须放在用户可写的位置。
...
| 终端/持久 shell 不可用，但文件读写正常 | macOS：`node-pty` 的 `spawn-helper` 缺可执行位。**Linux 上没有 spawn-helper**（helper 分支只编 `__APPLE__`，Linux 走 `forkpty()`），Linux 上出现这个症状要查 `prebuilds/linux-<arch>/pty.node` | macOS 确认 `node-pty/prebuilds/darwin-<arch>/spawn-helper` 是 `0755`；Linux 跑 `--doctor` 看原生模块加载结论，或 `node scripts/verify-cross-tree.mjs` 查平台包 |
...
| 多开窗口后出现两个宿主、日志 seq 撞号 | 宿主复用靠端口探测（`netstat.exe` 在非 Windows 上失效） | Linux/macOS 上用 `ss`/`lsof`；确认只有一个 `dsh web` 进程（`pgrep -af 'dsh.*web'`） |
...
> **不要**用 Windows 的记忆去猜 Linux 的路径或 macOS 的证书行为。
