...
- **桌面皮肤（三项界面改造 + 一处修正，纯注入 CSS，DSH 零改动）**：
  ...
  **刻意保留 8px 槽宽**（theme 插件的 `--dsh-scrollbar-width`）——若把宽度收到 0，正文会在悬停
  ...
  先绘制，天然落在标记条之下）。
  ③ **对话区底层黑遮罩**：覆盖**整个会话区**，`z-index:-1` 让它落在内容之下、页面底色之上，
  正文照常可读。（初版把它挂在了正文两侧的**拖动条**上，用户纠正："遮罩是覆盖整个对话页面，
  不是两个滑动栏"。选择器随之改为 `[class$="_root"]:has([class$="_scrollBody"]):not(:has([class$="_centerCol"]))`，
  并给该根加 `position:relative` 作为伪元素的定位基准。）
