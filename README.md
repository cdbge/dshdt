...
- **版本链**：v1（Node+Chrome）0.3.0 → Electron 0.4.0 → 0.4.1（多窗口复用/自愈/优雅退出/背景图）→ 0.4.2（背景图回环 HTTP 修复 + 新图标）→ 0.4.3（背景图穿透 rc.6 硬编码不透明层，像素级验证）→ 0.4.4（工作区选取钉住应用内浏览）→ **0.4.5（按钮悬停交互 + 阶段收尾，已打包）**；DSH 依赖锁定 0.1.0-rc.6
