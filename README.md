...
- **安装包**：`desktop-electron/dist/DSHDesktop-Setup-0.4.6.exe`（127.4 MB / 133,627,438 字节；未签名，SmartScreen 首次提示属预期）+ blockmap。
  ⚠️ **这份 exe 是 09-12 12:39 打的，不含当日晚些的"宿主可诊断性"加固**（stdio 两级化 + `--diag` + preflight）。
  含加固的代码已落在 `dist/win-unpacked`（冒烟 `SMOKE OK`），**要发人须先重打安装包**（`npm run dist`，需用户同意）；
  重打前的正确说法是"**发人就发 0.4.6，但先确认时间戳**"。
